...
class Mycost2
{
	public:
		...
		~Mycost2()
		{
			...
			int costdiff=(endt.tv_sec-m_begint.tv_sec)*1000+(endt.tv_usec-m_begint.tv_usec)/1000;
			printf(" cost %d\n", costdiff);
		}
	private:
		struct timeval m_begint;
};
...
public:
    ...
    static bool UncompressData1_V2(std::string& strData, uint32_t nHeadLen)
    {
        uLongf bufflen = 8 * (strData.size() - nHeadLen);
        if( bufflen > MAX_RECORD_SIZE ) {
            return false;
    ...
    static bool UncompressData2(std::string& strData, uint32_t nHeadLen)
    {
        ...
        if( nSize > MAX_RECORD_SIZE ) {
            return false;
        }
        
        // Modified:
        //uLongf buffLen = nSize + 51200;
...
public:
    ...
    void Copy2Inbuff(const std::string& data){
    	m_in = data;
    	m_inCursor = 0;
    }
    ...
    CPackData &operator >> (bool& bVal)
    {
        if (m_inCursor + 1 <= m_inPtr->length())
        {
            bVal = (bool)(*m_inPtr)[m_inCursor++];
        }
        else
        {
            throw PACK_LENGTH_ERROR;
    ...
    CPackData &operator >> (double& dVal)
    {
        if (m_inCursor + 8 <= m_inPtr->length())
        {
            uint64_t ullVal;
    ...
    void Copy2OutBuff(std::string& data){
    	m_out = data;
    	m_outCursor = 0;
    }

    void ReserveInBuff(uint32_t size){
    	m_in.reserve(size);
    }

    void ReserveOutBuff(uint32_t size){
    	m_out.reserve(size);
    }
    ...
    CPackData &operator << (const bool bVal)
    {
        if(m_outCursor < m_outPtr->length())
            (*m_outPtr)[m_outCursor] = (char)bVal;
        else
            (*m_outPtr) += (char)bVal;
        m_outCursor++;
        return *this;
    ...
    CPackData &operator << (const float fVal)
    {
        uint32_t dwVal;
        memcpy(&dwVal, &fVal, 4);
        return operator<<(dwVal);
    }
    ...
    CPackData &operator << (const double dVal)
    {
        uint64_t ullVal;
        memcpy(&ullVal, &dVal, 8);
        return operator<<(ullVal);
    }
    ...
    CPackData &operator << (const char* strVal)
    {
        uint32_t size = strlen(strVal);
        (*this) << size;
        m_outPtr->replace(m_outCursor, size, strVal);
        m_outCursor += size;
        return *this;
    }
