...
//  Tests
//
//  Created by wesley_chen on 2019/12/2.
...
@interface Tests_gzip : XCTestCase

@end

@implementation Tests_gzip
...
- (void)testOutputEqualsInput
{
    ...
    //compress
    NSData *compressedData = [WCDataTool gzippedDataWithData:inputData];

    //decode
    NSData *outputData = [WCDataTool gunzippedDataWithData:compressedData];
...
- (void)testUnzipNonZippedData
{
    ...
    //decode
    NSData *outputData = [WCDataTool gunzippedDataWithData:inputData];
    NSString *outputString = [[NSString alloc] initWithData:outputData encoding:NSUTF8StringEncoding];
    XCTAssertEqualObjects(outputString, inputString);
}
...
- (void)testRezipZippedData
{
    ...
    //compress again
    NSData *outputData = [WCDataTool gzippedDataWithData:compressedData];
    XCTAssertEqualObjects(compressedData, outputData);
}

- (void)testZeroLengthInput
{
    NSData *data = [WCDataTool gzippedDataWithData:[NSData data]];
    XCTAssertEqual(data.length, 0);

    data = [WCDataTool gunzippedDataWithData:[NSData data]];
    XCTAssertEqual(data.length, 0);
}
...
- (void)testCompressionPerformance
{
    ...
    [self measureBlock:^{
        __unused NSData *compressedData = [WCDataTool gzippedDataWithData:inputData];
    }];
}
...
- (void)testDecompressionPerformance
{
    ...
    NSData *compressedData = [WCDataTool gzippedDataWithData:inputData];
    [self measureBlock:^{
        __unused NSData *outputData = [WCDataTool gunzippedDataWithData:compressedData];
    }];
}
...
- (void)testEncodeWriteFileDecodeInShell
{
    ...
    NSData *gzippedData    = [WCDataTool gzippedDataWithData:inputData];
    XCTAssertEqualObjects(seggzippedData, gzippedData);

    // compare gunzipped SEGGZIPPED data to original data
    NSData *expandedSEGGZippedData = [WCDataTool gunzippedDataWithData:seggzippedData];
