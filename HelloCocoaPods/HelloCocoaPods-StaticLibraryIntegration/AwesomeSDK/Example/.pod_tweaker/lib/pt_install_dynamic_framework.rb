...
def add_codesnippet_for_install_dynamic_framework(pods_dir, target_name, pod_name_list)
  install_frameworks_script_name = 'Pods-' + target_name + '-frameworks.sh'
  path_of_install_frameworks_script = File.join(pods_dir, 'Target Support Files', 'Pods-' + target_name, install_frameworks_script_name)

  if File.exists?(path_of_install_frameworks_script)

    file_content = File.read(path_of_install_frameworks_script)

    pod_name_list.each do |pod_name|
