...
- (void)test_check_http_or_https_links {
    ...
    // Abnormal Case 1: ] not allowed in url
    string = @"https://ha.emas.alibaba-inc.com/#/page/crash?r_=/converge/detail/:appId&g_=[]";
    matches = [dataDetector matchesInString:string options:kNilOptions];
    XCTAssertTrue(matches.count == 1);
    result = [matches firstObject];
    matchString = [string substringWithRange:result.range];
    XCTAssertEqualObjects(matchString, @"https://ha.emas.alibaba-inc.com/#/page/crash?r_=/converge/detail/:appId&g_=[");
