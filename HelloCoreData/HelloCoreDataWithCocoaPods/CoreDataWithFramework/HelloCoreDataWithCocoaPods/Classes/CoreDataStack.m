...
- (NSURL *)modelURL {
    ...
    // Note: name for `s.resource_bundles` in podspec
    NSString *resourceBundleName = @"HelloCoreDataWithCocoaPods";
    
    do {
        // Check .momd in xxx.framework/yyy.bundle or main_bundle/yyy.bundle
        NSString *resourceBundlePath = [[NSBundle bundleForClass:self.class] pathForResource:resourceBundleName ofType:@"bundle"];
        NSBundle *resourceBundle = [NSBundle bundleWithPath:resourceBundlePath];
        URL = [resourceBundle URLForResource:self.modelName withExtension:@"momd"];
        if (URL) {
            break;
        }
        
        // Check .momd in xxx.framework or main bundle
        URL = [[NSBundle bundleForClass:self.class] URLForResource:self.modelName withExtension:@"momd"];
        if (URL) {
            break;
        }
    } while (NO);
